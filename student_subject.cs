...
namespace ExampleCAdvance.Entities
{
    public class Student
    {
        ...
        [EmailAddress(ErrorMessage = "Invalid email (vi du: ten@domain.com)")]
    ...
    public class Program
    {
        ...
        static readonly string DataFile =
            Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "students_data.txt");
        ...
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                Console.WriteLine("11. Xoa toan bo du lieu");
        ...
        static void SaveData()
        {
            ...
            foreach (var s in subject.Students)
            {
                lines.Add(string.Join("|", new string[] {
                    s.StuId ?? "",
                    s.Name ?? "",
                    s.MidPoint.ToString(System.Globalization.CultureInfo.InvariantCulture),
                    s.FinalPoint.ToString(System.Globalization.CultureInfo.InvariantCulture),
                    s.MathPoint.ToString(System.Globalization.CultureInfo.InvariantCulture),
                    s.Email ?? ""
                }));
            }
        ...
        static void LoadData()
        {
            ...
            string[] h = lines[0].Trim().Split('|');
            if (h.Length >= 4)
            {
                subject.SubID = h[0];
                subject.Name = h[1];
                int credit;
                if (int.TryParse(h[2], out credit))
            ...
            for (int i = 1; i < lines.Length; i++)
            {
                ...
                subject.Students.Add(new Student
                {
                    StuId = p[0],
                    Name = p[1],
                    MidPoint = ParseDouble(p[2]),
                    FinalPoint = ParseDouble(p[3]),
                    MathPoint = ParseDouble(p[4]),
                    Email = p[5]
                });
            }
        }
        ...
        static void ResetData()
        {
            subject.Students.Clear();
            SaveData();
            Console.WriteLine("Da xoa toan bo du lieu!");
        }
